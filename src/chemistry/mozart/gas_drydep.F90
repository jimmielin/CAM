module gas_drydep
  ...
  real(r8) :: vonkar    = huge(1._r8)  ! von Karman constant
  real(r8) :: tmelt     = huge(1._r8)  ! freezing point of water (K)
  ...
  real(r8), allocatable :: z0(:,:)     ! roughness length (season, land type)
  real(r8), allocatable :: rgso(:,:)   ! ground resistance, O3
  real(r8), allocatable :: rgss(:,:)   ! ground resistance, SO2
  real(r8), allocatable :: ri(:,:)     ! richardson number based resistance
  real(r8), allocatable :: rclo(:,:)   ! lower canopy resistance, O3
  real(r8), allocatable :: rcls(:,:)   ! lower canopy resistance, SO2
  real(r8), allocatable :: rlu(:,:)    ! leaf cuticle resistance
  real(r8), allocatable :: rac(:,:)    ! in-canopy aerodynamic resistance
  real(r8), allocatable :: foxd(:)     ! oxidation reactivity factor per dep species
  real(r8), allocatable :: drat(:)     ! sqrt of molecular weight ratio per dep species
...
contains
  ...
  subroutine gas_drydep_init( gas_pcnst_in, plev_in, karman_in, tmelt_in, &
    ...
    real(r8),         intent(in)  :: z0_in(:,:)
    real(r8),         intent(in)  :: rgso_in(:,:)
    real(r8),         intent(in)  :: rgss_in(:,:)
    real(r8),         intent(in)  :: ri_in(:,:)
    real(r8),         intent(in)  :: rclo_in(:,:)
    real(r8),         intent(in)  :: rcls_in(:,:)
    real(r8),         intent(in)  :: rlu_in(:,:)
    real(r8),         intent(in)  :: rac_in(:,:)
    real(r8),         intent(in)  :: foxd_in(:)
    real(r8),         intent(in)  :: drat_in(:)
...
! =============================
  ...
  subroutine gas_drydep_run( ocnfrac, icefrac, ocnfrc_x, icefrc_x, sfc_temp, pressure_sfc,  &
    ...
    real(r8), intent(in)      :: ocnfrc_x(:)
    real(r8), intent(in)      :: icefrc_x(:)
    integer,  intent(in)      :: ncol
    real(r8), intent(in)      :: sfc_temp(:)          ! surface temperature (K)
    real(r8), intent(in)      :: pressure_sfc(:)      ! surface pressure (Pa)
    real(r8), intent(in)      :: wind_speed(:)        ! 10 meter wind speed (m/s)
    real(r8), intent(in)      :: spec_hum(:)          ! specific humidity (kg/kg)
    real(r8), intent(in)      :: air_temp(:)          ! surface air temperature (K)
    real(r8), intent(in)      :: pressure_10m(:)      ! 10 meter pressure (Pa)
    real(r8), intent(in)      :: rain(:)
    real(r8), intent(in)      :: snow(:)              ! snow height (m)
    real(r8), intent(in)      :: solar_flux(:)        ! direct shortwave radiation at surface (W/m^2)
    real(r8), intent(in)      :: tv(:)                ! potential temperature
    real(r8), intent(in)      :: lnd_dvel(:,:)        ! deposition velocity over land (cm/s)
    real(r8), intent(in)      :: heff(:,:)            ! effective Henry's law coefficients (M/atm)
    real(r8), intent(in)      :: mmr(:,:,:)           ! constituent concentration (kg/kg)
    real(r8), intent(out)     :: dvelocity(:,:)       ! deposition velocity (cm/s)
    real(r8), intent(inout)   :: dflx(:,:)            ! deposition flux (/cm^2/s)

    !-------------------------------------------------------------------------------------
    ! 	... local variables
    !-------------------------------------------------------------------------------------
    real(r8) :: ocnice_dvel(ncol,gas_pcnst)
    real(r8) :: ocnice_dflx(ncol,gas_pcnst)

    real(r8), dimension(ncol) :: term    ! work array
    integer  :: ispec
    real(r8)  :: lndfrac(ncol)
    integer :: i
...
! ========================================================================
  ...
  subroutine drydep_xactive( sfc_temp, pressure_sfc,  &
    ...
    real(r8), intent(in)      :: tv(:)                ! potential temperature
    real(r8), intent(in)      :: mmr(:,:,:)           ! constituent concentration (kg/kg)
    real(r8), intent(out)     :: dvel(:,:)            ! deposition velocity (cm/s)
    real(r8), intent(inout)   :: dflx(:,:)            ! deposition flux (/cm^2/s)

    integer, intent(in), optional     ::  beglandtype
    integer, intent(in), optional     ::  endlandtype

    real(r8), intent(in), optional      :: ocnfrc(:)
    real(r8), intent(in), optional      :: icefrc(:)
    ...
    !-------------------------------------------------------------------------------------
    real(r8), dimension(ncol,n_land_type) :: dep_ra ! [s/m] aerodynamic resistance
    real(r8), dimension(ncol,n_land_type) :: dep_rb ! [s/m] resistance across sublayer
    ...
    do lt = beglt,endlt
       do i = 1,ncol
          if( fr_lnduse(i,lt) ) then
             ...
             dep_rb (i,lt) = (2._r8/(vonkar*ustar(i,lt))) * crb
             rds(i,lt) = 1._r8/vds
          end if
       end do
    end do
       ...
       if( has_dvel(ispec) ) then
          ...
          lt_loop: do lt = beglt,endlt
             ...
             case( 'NH4', 'NH4NO3', 'XNH4NO3' )
                where( fr_lnduse(:ncol,lt) )
                   wrk(:) = wrk(:) + lnd_frc(:)/(dep_ra(:ncol,lt) + 0.5_r8*rds(:,lt))
                endwhere
