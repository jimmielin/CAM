...
!
  ...
  real(r8), allocatable              :: dheff(:,:)  ! effective Henry's law table (dep data file)
...
!
  ...
  real(r8),       intent(in)    :: mmr(:,:,:)               ! mass mixing ratio (kg/kg)
  real(r8),       intent(in)    :: pmid(:,:)                ! midpoint pressures (Pa)
  real(r8),       intent(in)    :: pdel(:,:)                ! pressure delta about midpoints (Pa)
  real(r8),       intent(in)    :: zint(:,:)                ! interface geopotential height above the surface (m)
  real(r8),       intent(in)    :: tfld(:,:)                ! midpoint temperature (K)
...
!
  ...
  real(r8),       intent(in)    :: area(:)                  ! cell area (m^2)
  real(r8),       intent(in)    :: lats(:)                  ! latitudes (radians)
  integer,        intent(in)    :: mapping_to_mmr(:)        ! wetdep species -> constituent index
  real(r8),       intent(in)    :: mol_weight(:)            ! wetdep species molecular weight (g/mol)
  integer,        intent(in)    :: index_cldice             ! CLDICE constituent index
  integer,        intent(in)    :: index_cldliq             ! CLDLIQ constituent index
  real(r8),       intent(inout) :: wd_tend(:,:,:)
  real(r8),       intent(inout) :: wd_tend_int(:,:)
  ...
  real(r8),       intent(out)   :: dtwr(:,:,:)
  real(r8),       intent(out)   :: heff(:,:,:)
  real(r8),       intent(out)   :: qt_rain(:,:)
  real(r8),       intent(out)   :: qt_rime(:,:)
  real(r8),       intent(out)   :: qt_wash(:,:)
  real(r8),       intent(out)   :: qt_evap(:,:)
!
! local arrays and variables
!
  integer :: i,k,l,kk,m
  real(r8), parameter                       :: gravit = SHR_CONST_G         ! m/s^2
  real(r8), dimension(ncol)                 :: wk_out
  ...
  real(r8), dimension(ncol,pver,gas_wetdep_cnt) :: trc_mass
